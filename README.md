...
1. 运行**data_util.py**文件，因为pytorch版本的训练速度比较慢，所以我为了节省时间，在原有数据集的基础上只选择了**100**条来划分训练集合测试集（就这训练了30个epoch都花了将近一个半小时），完整的数据一共是**2000**条。

2. 进入pytorch文件夹下的**train.py**文件运行就可以了
3. 每次训练的模型结果都保留在**pytorch/model**文件夹下，我把30次训练的模型文件都上传上去啦
4. 后面打算先加批处理，尽量提高一下训练速度，再看怎么调参数吧